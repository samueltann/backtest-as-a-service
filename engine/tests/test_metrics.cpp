...
TEST_CASE("days_between handles month and leap-year boundaries") {
    REQUIRE(days_between(Date::parse("2024-01-01"), Date::parse("2024-01-05")) == 4);
    REQUIRE(days_between(Date::parse("2024-02-28"), Date::parse("2024-03-01")) == 2);   // leap year
    REQUIRE(days_between(Date::parse("2023-02-28"), Date::parse("2023-03-01")) == 1);
    REQUIRE(days_between(Date::parse("2015-01-01"), Date::parse("2025-01-01")) == 3653);
}

TEST_CASE("metrics from a hand-computed equity curve and trade log") {
    std::vector<EquityPoint> curve = {
        {Date::parse("2024-01-01"), 100.0}, {Date::parse("2024-01-02"), 110.0},
        {Date::parse("2024-01-03"), 99.0},  {Date::parse("2024-01-04"), 104.5},
        {Date::parse("2024-01-05"), 121.0},
    };
    // Trade dates are unused by metrics (only pnl/return_pct matter here).
...
TEST_CASE("flat equity curve produces zero-risk metrics without dividing by zero") {
    std::vector<EquityPoint> curve = {
        {Date::parse("2024-01-01"), 100.0}, {Date::parse("2024-01-02"), 100.0},
        {Date::parse("2024-01-03"), 100.0}};
